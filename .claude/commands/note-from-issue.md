...
3. **全绿才合并**（合并即触发 deploy、note 直接上线 xdanger.com，接近不可逆）。门禁见第 6 步。
   **门禁不满足绝不强合**——停下、留 `note-in-progress`、评论待人工，进下一个 issue。
...
```bash
mkdir -p .note-intake
# 新 issue：有 note-taking、没 note-in-progress、author 为 xdanger、状态 open
gh issue list --repo daihaus/xdanger.com --state open --author xdanger \
  --label note-taking --json number,title,labels,createdAt \
  --search '-label:note-in-progress' > .note-intake/new.json
# 在途 issue（崩溃恢复）：有 note-in-progress 的，续跑
gh issue list --repo daihaus/xdanger.com --state open --author xdanger \
  --label note-taking --label note-in-progress \
  --json number,title,labels,createdAt > .note-intake/resume.json
```
...
- workflow 返回 `pass=false`（2 轮后仍有 blocker）→ **不建 PR**。**只**撤掉本 issue 的草稿：
  逐个遍历 workflow 返回的 `filesChanged`，已跟踪文件 `git checkout -- <file>`、未跟踪新文件 `rm`；
  **绝不用 `git checkout -- .` / `git reset --hard`**（会清掉无关 WIP）。保留 `note-in-progress`
  并在 issue 上评论说明 blocker、@xdanger 人工跟进，然后处理下一个 issue。
- `pass=true` → 继续。

### 4. 本地构建验证（硬门禁，仓库没有 PR CI build）

```bash
pnpm build:site
```

- 失败 → 同上：评论失败原因、保留 `note-in-progress`、跳过本 issue 等人工。
- 通过 → 继续。
...
git add <workflow 返回的 filesChanged 列表>   # 精确 stage，绝不 git add -A（防带入 .note-intake/ 等）
```
...
until
  ...
  done_cr=$(gh run list --repo daihaus/xdanger.com --workflow "Code Review" \
    --json headSha,status,conclusion --jq --arg h "$head" \
    'map(select(.headSha==$h)) | (length>0) and all(.[]; .status=="completed")
...
```bash
gh pr merge <pr> --repo daihaus/xdanger.com --merge --delete-branch   # merge commit（全局规则）+ 删本地&远程分支
git switch main && git pull --ff-only
git branch -D note/issue-<n> 2>/dev/null || true   # --delete-branch 通常已删本地；专用 worktree 下删不掉时兜底，缺失不算错
gh issue edit <n> --repo daihaus/xdanger.com --add-label note-published --remove-label note-in-progress
gh issue view <n> --repo daihaus/xdanger.com --json state   # PR 的 "Closes #<n>" 应已自动 close；
# 若仍 open：gh issue close <n> --reason completed
...
> `delete_branch_on_merge` 仓库设为 false，所以 `--delete-branch` 必须显式带上才删远程分支
> （该 flag 会删本地 + 远程；本地若已被删掉，上面 `git branch -D` 用 `|| true` 兜底不报错）。

处理完一个 issue 后，回到第 1 步处理下一个；都处理完则本轮结束。

### 8. 恢复（`resume.json` 里的在途 issue）

这些 issue 已 `note-in-progress`，可能已有分支/PR。**别重复建**。先查**所有状态**的对应 PR
（含已合并——覆盖「合并后、翻 label 前崩溃」的窗口，否则会给已上线的内容再建第二个 PR）：
...
1. **已有 MERGED 的 PR** → note 其实已发布，只是没收尾：**绝不重建**，直接补跑第 7 步收尾
   （翻 `note-published` / 移除 `note-in-progress`、issue 仍 open 则 `gh issue close`、评论已发布
   URL、`rm .note-intake/issue-<n>.json`）。
